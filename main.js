...
const containerConfigurador = document.getElementById("veiwPortContainer");
...
function createPart(
  ...
  const geometryPart = new THREE.BoxGeometry(
    width / 1000,
    length / 1000,
    thickness / 1000
  ); // create a panel geometry with dimentions converted from millimeters, 1 scene unit as 1000mm
...
//Loading Handler obj with MTLloader
    ...
    objLoader.load(
      ...
      function (object) {
        ...
        createWardrobe(
          ...
          InitialDoorThickness,
          InitialBackThickness,
          material,
          InitialFooterHeight,
          InitialDoorGap,
          InitialFooterGap
...
function createWardrobe(
  ...
  if (length <= 600) {
    ...
    const doorPartA = createPart(
      0.5 * length - 1.5 * doorGap,
      height - doorGap - footerHeight,
      doorThickness,
      secondaryMaterial,
    ...
    const innerSidePart = createPart(
    ...
    const shelfPart = createPart(
      (length - 3 * materialThickness) / 2,
      width - materialThickness,
      materialThickness,
      material,
    ...
    const shelfPart = createPart(
      (length - 2 * materialThickness) / 3 - 0.5 * materialThickness,
      width - materialThickness,
      materialThickness,
      material,
      ((1 / 3) * length - (1 / 4) * doorGap) * scalePositionCorrection,
  ...
  const backFooterPart = createPart(
    length - 2 * materialThickness,
    footerHeight,
    materialThickness,
    material,
  ...
  const frontFooterPart = createPart(
    length - 2 * materialThickness,
    footerHeight,
    materialThickness,
    secondaryMaterial,
